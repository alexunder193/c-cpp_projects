...
{/* Pro: 		to paidi deixnei ena kombo se ena AVL dentro	
  *	Meta:		epistrefei ton patera ston "pateras" tou kombou pou deixnei to paidi */      
    ...
    if(AVLTree_keno(riza)||AVLTree_keno(paidi)||TStree_iso(paidi->dedomena, riza->dedomena))
    {   *pateras=NULL;
		*error=1;
		return;
	}
	if(paidi==riza->apaidi||paidi==riza->dpaidi)
		*pateras=riza;
	else if(TStree_mikrotero(paidi->dedomena, riza->dedomena))
		AVLTree_pateras(riza->apaidi, pateras, paidi, error);
	else if(TStree_megalytero(paidi->dedomena, riza->dedomena))
...
{/* Pro: 		Dimiourgia enos AVL dentrou 
  ...
  *             to dentro na parameinei isozigismeno*/
	...
	{  	*riza = malloc(sizeof(struct typos_komvou));
		...
		TStree_setValue(&((*riza)->dedomena),stoixeio);
	...
	else if (TStree_iso(stoixeio,(*riza)->dedomena)) 
    {   /* printf("O komvos yparxei idi sto AVL-dentro!\n"); */
		*error=1;
	} else if (TStree_mikrotero(stoixeio,(*riza)->dedomena)) 
	...
	if (parametros=='e'){ /*periptosi opou i sinartisi exei klithei gia eisagogi kombou*/
		...
		case AY:/* apli LL peristrofi*/
            ...
            if(printinfo){  
		    	...
		    	TStree_writeValue(stdout, (*riza)->dedomena);
             }
		...
		case DY:/*dipli LR  peristrofi */
            ...
            if(printinfo){   
              //printf("aristeri peristrofi: ",x->dedomena);
 	          TStree_writeValue(stdout, x->dedomena);
            }
...
{/*	Pro: 		ï *p deixnei tin riza tou ipodentrou pou tha ginei aristeri peristrofi
...
{/*	Pro: 		Dimiourgia enos AVL dentrou
  *	Meta:		Diagrafi tou kombou me periexomeno "stoixeio"
	...
	*error=0;
	...
	else if (TStree_mikrotero(stoixeio, (*riza)->dedomena) )/*anazitisi sto aristero ipodentro*/
	...
	else if (TStree_megalytero(stoixeio,(*riza)->dedomena)) /*anazitisi sto deksi ipodentro*/
	...
	{   prosorinos=(*riza);
		...
		else if( prosorinos->apaidi == NULL) /*o kombos den exei deksi paidi*/
		{   (*riza)=prosorinos->dpaidi;
			*ypsilotero=TRUE;
			free(prosorinos);
		}
		else /*ï kombos exei 2 paidia */
		{   /*to periexomeno tou kombou pros diagrafi antikathistatai apo to deksiotero stoixeio tou aristerou 
		      tou paidiou*/
...
void swap(typos_deikti *riza,typos_deikti *temp,int *ypsilotero){
	...
	{   delnode=(*riza);
		TStree_setValue(&((*temp)->dedomena),(*riza)->dedomena);
		(*riza)=(*riza)->apaidi;
		if(printinfo)
        { //printf("swap with [ ");
          TStree_writeValue(stdout, delnode->dedomena);
           //printf("]\n");
        }
		free(delnode);
		*ypsilotero=TRUE;
...
{/*	Pro: 		Dimiourgia enos AVL dentrou
  *	Meta:		epistrefei ena deikti ston kombo pou periexei "stoixeio" diaforetika NULL */
    *found=0;
	...
	if (TStree_iso(stoixeio, riza->dedomena)) 
    {   *deiktis=riza;                     
		*found=1;
    }
	else if (TStree_mikrotero(stoixeio,riza->dedomena) ) /*anazitisi sto aristero ipodentro*/
 ...
 *	Meta:	    Epistrefei sto "stoixeio" tin timi ton dedomenon pou iparxoun ston kombo pou deixnei i "riza"*/     
   *error=0;  
     if(!AVLTree_keno(riza))
        ...
        TStree_setValue(stoixeio,riza->dedomena);
    else
        *error=1;     
}
...
{/*	Ðño: 	    Dimiourgia enos AVL dentrou 
  *	Meta:	    Enimenonei ton kombo pou exei tin timi old allazontas tin timi se new */
    typos_deikti deiktis;
	int found ,ypsilotero;
	ypsilotero=0;
	*error=found=0;
	deiktis=NULL;
	
	AVLTree_anazitisi(*tree, old, &deiktis, &found);
	if(found)
	{   AVLTree_diagrafi(tree, old,&ypsilotero, error);
		if(!(*error))
			AVLTree_eisagogi(tree, new,&ypsilotero, error);
	}
	else
		*error=1; 
